...
| Opus decoder                              | 🟢 (Phase 1 + Phase 2a/2b/2c.1/2c.2a/2c.2b/2c.3a/2c.3b.1/2c.3b.2/2c.3b.3/2c.3b.4/2c.3b.5a/2c.3b.5b/2c.3b.5c/2c.3b.5d/2c.4 — framing, range coder, CELT foundation + coarse energy + tf/spread + init_caps/dyn_alloc/alloc_trim + compute_allocation (intensity/dual stereo/skip) + unquant_fine_energy + PVQ math helpers + pulse cache + PVQ shape codeword decoder (`decode_pulses`, `cwrsi`) + energy split decoder (`compute_qn`, `compute_theta`, `quant_band_n1`) + band shape primitives (`haar1`, `hadamard`, `exp_rotation`, `normalise_residual`, `extract_collapse_mask`, `alg_unquant`) + `quant_partition` recursive mono splitter + `renormalise_vector` + `celt_lcg_rand` + `quant_band` mono wrapper (recombine / time-divide / Hadamard reorganisation + √N lowband_out scaling) + `quant_band_stereo` mid/side band wrapper + `stereo_merge` inverse-rotation L/R reconstructor + `quant_all_bands` top-level band-iteration driver (per-band budget, lowband fold-source bookkeeping, conservative collapse-mask accumulator, dual-stereo intensity crossover, mono/stereo dispatch) + `special_hybrid_folding` hybrid-second-band duplicator + `anti_collapse` short-block noise injection + `unquant_energy_finalise` final fine-energy refinement shipped; see [`src/Mediar.Codecs.Opus.Decoder/README.md`](src/Mediar.Codecs.Opus.Decoder/README.md). Phases 2d–6 deliver CELT IMDCT, SILK, hybrid, resampler, multistream.) |
