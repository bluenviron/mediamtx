...
| Opus decoder                              | 🟢 (Phase 1 + Phase 2a/2b/2c.1/2c.2a/2c.2b/2c.3a/2c.3b.1/2c.3b.2/2c.3b.3/2c.3b.4/2c.3b.5a — framing, range coder, CELT foundation + coarse energy + tf/spread + init_caps/dyn_alloc/alloc_trim + compute_allocation (intensity/dual stereo/skip) + unquant_fine_energy + PVQ math helpers + pulse cache + PVQ shape codeword decoder (`decode_pulses`, `cwrsi`) + energy split decoder (`compute_qn`, `compute_theta`, `quant_band_n1`) + band shape primitives (`haar1`, `hadamard`, `exp_rotation`, `normalise_residual`, `extract_collapse_mask`, `alg_unquant`) + `quant_partition` recursive mono splitter + `renormalise_vector` + `celt_lcg_rand` shipped; see [`src/Mediar.Codecs.Opus.Decoder/README.md`](src/Mediar.Codecs.Opus.Decoder/README.md). Phases 2c.3b.5b–6 deliver CELT `quant_band` / `quant_all_bands` / anti-collapse / IMDCT, SILK, hybrid, resampler, multistream.) |
