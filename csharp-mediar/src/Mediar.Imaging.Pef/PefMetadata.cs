...
/// <summary>
/// Pentax-specific metadata parsed from the PEF file (mostly EXIF / IFD 0 tags).
/// </summary>
public sealed record PefMetadata
{
    /// <summary>EXIF Make ("PENTAX" or "RICOH IMAGING COMPANY, LTD." for genuine PEF files).</summary>
    public required string? Make { get; init; }

    /// <summary>EXIF Model (e.g. "PENTAX K-3 Mark III").</summary>
    ...
    /// <summary>Number of bytes occupied by the raw Pentax MakerNote (tag 0x927C), or 0 if absent.</summary>
    public required int MakerNoteLength { get; init; }
}

/// <summary>Public view of a single PEF sub-image (typically IFD 0 plus one SubIFD per pyramid level).</summary>
public sealed record PefSubImageInfo
{
    ...
    /// 65535 = Pentax PEF compressed (proprietary 12-bit Huffman, not yet supported).
    /// </summary>
