...
/// <summary>
/// Mamiya-specific metadata parsed from the MEF file (mostly EXIF / IFD 0 tags).
/// </summary>
public sealed record MefMetadata
{
    /// <summary>EXIF Make ("Mamiya", "MAMIYA" or "Phase One" for genuine MEF files).</summary>
    ...
    /// <summary>Number of bytes occupied by the raw Mamiya MakerNote (tag 0x927C), or 0 if absent.</summary>
    public required int MakerNoteLength { get; init; }
}

/// <summary>Public view of a single MEF sub-image (typically IFD 0 plus one SubIFD per pyramid level).</summary>
public sealed record MefSubImageInfo
{
    ...
    /// 65000 = Mamiya-compressed RAW (proprietary, not yet supported).
    /// </summary>
