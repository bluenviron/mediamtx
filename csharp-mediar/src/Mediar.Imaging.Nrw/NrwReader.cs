...
namespace Mediar.Imaging.Nrw;

/// <summary>
/// Reader for Nikon Coolpix RAW (NRW) files. NRW is TIFF-based, identified at
/// parse time by the EXIF <c>Make</c> tag value beginning with
/// "NIKON" or "Nikon" (
/// Coolpix P6000 / P7000 / P7100 / P7700 / P7800 and Nikon A APS-C compact
/// ). The reader composes <see cref="TiffReader"/> for pixel
/// decode and exposes the parsed Nikon-specific metadata block.
/// </summary>
/// <remarks>
/// <para>
/// Like NEF / ARW / PEF / DCR, NRW places a small preview / thumbnail in
...
/// </para>
/// <para>
/// Nikon-compressed RAW (TIFF compression tag 34713) uses the same proprietary predictor + Huffman scheme as NEF and is not yet implemented. NRW is structurally identical to NEF but emitted by Nikon Coolpix and the Nikon A APS-C compact rather than the DSLR / Z mirrorless line, so the file extension is the sole disambiguator from header bytes alone.
/// 
...
/// </para>
/// </remarks>
public sealed class NrwReader : IImageReader
{
    ...
    /// <inheritdoc/>
    public ImageFormat Format => ImageFormat.Nrw;
    ...
    /// <summary>The Nikon-specific metadata block parsed from IFD 0.</summary>
    public NrwMetadata NRW { get; }

    /// <summary>All sub-images discovered in this NRW file (IFD 0 plus SubIFDs in walk order).</summary>
    public IReadOnlyList<NrwSubImageInfo> SubImages { get; }

    private NrwReader(Stream s, bool ownsStream, byte[] bytes, bool le,
                     ImageInfo info, ImageMetadata meta, NrwMetadata nikon,
                     IReadOnlyList<NrwSubImageInfo> subImages, bool canDecode)
    {
        _stream = s; _ownsStream = ownsStream;
        _bytes = bytes; _littleEndian = le;
        Info = info; Metadata = meta; NRW = nikon;
        SubImages = subImages; CanDecodePixels = canDecode;
    }

    /// <summary>Open a NRW file by path.</summary>
    public static NrwReader Open(string path)
    {
    ...
    /// <summary>Open a NRW from a stream (the contents are buffered into memory).</summary>
    public static NrwReader Open(Stream stream, bool ownsStream = false)
    {
        ArgumentNullException.ThrowIfNull(stream);
        using var ms = new MemoryStream();
        stream.CopyTo(ms);
        var bytes = ms.ToArray();
        if (bytes.Length < 8) throw new ImageFormatException("Truncated NRW.");

        bool le = bytes[0] == 'I' && bytes[1] == 'I';
        bool be = bytes[0] == 'M' && bytes[1] == 'M';
        if (!le && !be) throw new ImageFormatException("Bad NRW byte-order mark (expected II or MM).");
        int magic = ReadU16(bytes, 2, le);
        if (magic != 42) throw new ImageFormatException("Unsupported NRW/TIFF magic " + magic + ".");

        uint ifd0Offset = ReadU32(bytes, 4, le);
        if (ifd0Offset == 0) throw new ImageFormatException("NRW file has no IFDs.");

        var ifd0 = ParseIfd(bytes, le, (int)ifd0Offset);
        var nikon = ParseNrwMetadata(ifd0, bytes, le);

        if (string.IsNullOrEmpty(nikon.Make) || !IsNikonMake(nikon.Make))
        {
            throw new ImageFormatException(
                "Not a NRW file (EXIF Make tag does not identify a Nikon camera).");
        }

        var subs = new List<NrwSubImageInfo>();
        ...
        var info = new ImageInfo
        {
            ...
            Format = ImageFormat.Nrw,
            HasAlpha = false,
            FrameCount = 1,
            ColorSpace = "RAW",
        };

        var meta = BuildImageMetadata(nikon);
        return new NrwReader(stream, ownsStream, bytes, le, info, meta, nikon,
                             subs, primary.CanDecodePixels);
    }

    private static bool IsNikonMake(string make) =>
        make.StartsWith("NIKON", StringComparison.Ordinal) || make.StartsWith("Nikon", StringComparison.Ordinal);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        if (!CanDecodePixels)
        {
            throw new NotSupportedException(
                "This NRW file's primary image uses an unsupported compression scheme " +
                "(Nikon-compressed RAW / tag 34713 is not yet implemented).");
        }
        ...
        {
            throw new NotSupportedException(
                "NRW primary image was not produced by the underlying TIFF decoder.");
        }
    }
    ...
    private static NrwSubImageInfo SelectPrimary(IReadOnlyList<NrwSubImageInfo> subs)
    {
        NrwSubImageInfo? best = null;
        long bestPixels = 0;
        ...
        return best ?? throw new ImageFormatException("NRW file has no inspectable sub-images.");
    }

    private static void WalkIfdsRecursive(byte[] bytes, bool le, uint ifdOffset,
                                          int parentSubIfdLevel,
                                          List<NrwSubImageInfo> sink,
    ...
    private static NrwSubImageInfo BuildSubImageInfo(IfdEntry[] entries, int subIfdLevel, byte[] bytes, bool le)
    {
        ...
        // Nikon-compressed RAW = 34713; not yet supported.
    ...
    private static NrwMetadata ParseNrwMetadata(IfdEntry[] entries, byte[] bytes, bool le)
    {
    ...
    private static ImageMetadata BuildImageMetadata(NrwMetadata nikon)
    {
        var tags = new Dictionary<string, string>(StringComparer.Ordinal);
        if (nikon.MakerNoteLength > 0)
        {
            tags["NRW:MakerNoteLength"] = nikon.MakerNoteLength.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        return new ImageMetadata
        {
            CameraMake = nikon.Make,
            CameraModel = nikon.Model,
            Software = nikon.Software,
            CapturedAtRaw = nikon.DateTime,
            Author = nikon.Artist,
            Copyright = nikon.Copyright,
