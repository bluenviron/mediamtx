...
/// <summary>
/// Epson-specific metadata parsed from the ERF file (mostly EXIF / IFD 0 tags).
/// </summary>
public sealed record ErfMetadata
{
    /// <summary>EXIF Make ("EPSON", "Epson" or "SEIKO EPSON" for genuine ERF files).</summary>
    ...
    /// <summary>Number of bytes occupied by the raw Epson MakerNote (tag 0x927C), or 0 if absent.</summary>
    public required int MakerNoteLength { get; init; }
}

/// <summary>Public view of a single ERF sub-image (typically IFD 0 plus one SubIFD per pyramid level).</summary>
public sealed record ErfSubImageInfo
{
    ...
    /// 65535 = Epson-compressed RAW (proprietary, not yet supported).
    /// </summary>
