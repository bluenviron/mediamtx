...
/// <summary>
/// Reader for Epson R-D1 rangefinder RAW (ERF) files. ERF is TIFF-based, identified at
/// parse time by the EXIF <c>Make</c> tag value beginning with
/// "EPSON", "Epson" or "SEIKO EPSON" (
/// Epson R-D1, R-D1s, R-D1x rangefinder cameras (the only Epson digital camera line that ever shipped a RAW format)
/// ). The reader composes <see cref="TiffReader"/> for pixel
/// decode and exposes the parsed Epson-specific metadata block.
/// </summary>
/// <remarks>
/// <para>
/// Like NEF / ARW / PEF / DCR, ERF places a small preview / thumbnail in
...
/// </para>
/// <para>
/// Epson-proprietary compression (TIFF compression tag 65535 in the wild). Some R-D1 bodies emit uncompressed 12-bit packed CFA mosaics in IFD 0; later firmware releases used a custom Epson packing scheme that Mediar does not yet decode.
/// 
...
/// </para>
/// </remarks>
public sealed class ErfReader : IImageReader
{
    ...
    /// <inheritdoc/>
    public ImageFormat Format => ImageFormat.Erf;
    ...
    /// <summary>The Epson-specific metadata block parsed from IFD 0.</summary>
    public ErfMetadata ERF { get; }

    /// <summary>All sub-images discovered in this ERF file (IFD 0 plus SubIFDs in walk order).</summary>
    public IReadOnlyList<ErfSubImageInfo> SubImages { get; }

    private ErfReader(Stream s, bool ownsStream, byte[] bytes, bool le,
                     ImageInfo info, ImageMetadata meta, ErfMetadata epson,
                     IReadOnlyList<ErfSubImageInfo> subImages, bool canDecode)
    {
        _stream = s; _ownsStream = ownsStream;
        _bytes = bytes; _littleEndian = le;
        Info = info; Metadata = meta; ERF = epson;
        SubImages = subImages; CanDecodePixels = canDecode;
    }

    /// <summary>Open a ERF file by path.</summary>
    public static ErfReader Open(string path)
    {
    ...
    /// <summary>Open a ERF from a stream (the contents are buffered into memory).</summary>
    public static ErfReader Open(Stream stream, bool ownsStream = false)
    {
        ArgumentNullException.ThrowIfNull(stream);
        using var ms = new MemoryStream();
        stream.CopyTo(ms);
        var bytes = ms.ToArray();
        if (bytes.Length < 8) throw new ImageFormatException("Truncated ERF.");

        bool le = bytes[0] == 'I' && bytes[1] == 'I';
        bool be = bytes[0] == 'M' && bytes[1] == 'M';
        if (!le && !be) throw new ImageFormatException("Bad ERF byte-order mark (expected II or MM).");
        int magic = ReadU16(bytes, 2, le);
        if (magic != 42) throw new ImageFormatException("Unsupported ERF/TIFF magic " + magic + ".");

        uint ifd0Offset = ReadU32(bytes, 4, le);
        if (ifd0Offset == 0) throw new ImageFormatException("ERF file has no IFDs.");

        var ifd0 = ParseIfd(bytes, le, (int)ifd0Offset);
        var epson = ParseErfMetadata(ifd0, bytes, le);

        if (string.IsNullOrEmpty(epson.Make) || !IsEpsonMake(epson.Make))
        {
            throw new ImageFormatException(
                "Not a ERF file (EXIF Make tag does not identify a Epson camera).");
        }

        var subs = new List<ErfSubImageInfo>();
        ...
        var meta = BuildImageMetadata(epson);
        return new ErfReader(stream, ownsStream, bytes, le, info, meta, epson,
                             subs, primary.CanDecodePixels);
    }

    private static bool IsEpsonMake(string make) =>
        make.StartsWith("EPSON", StringComparison.Ordinal) || make.StartsWith("Epson", StringComparison.Ordinal) || make.StartsWith("SEIKO EPSON", StringComparison.Ordinal);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        if (!CanDecodePixels)
        {
            throw new NotSupportedException(
                "This ERF file's primary image uses an unsupported compression scheme " +
                "(Epson-compressed RAW / tag 65535 is not yet implemented).");
        }
        ...
        {
            throw new NotSupportedException(
                "ERF primary image was not produced by the underlying TIFF decoder.");
        }
    }
    ...
    private static ErfSubImageInfo SelectPrimary(IReadOnlyList<ErfSubImageInfo> subs)
    {
        ErfSubImageInfo? best = null;
        long bestPixels = 0;
        ...
        return best ?? throw new ImageFormatException("ERF file has no inspectable sub-images.");
    }

    private static void WalkIfdsRecursive(byte[] bytes, bool le, uint ifdOffset,
                                          int parentSubIfdLevel,
                                          List<ErfSubImageInfo> sink,
    ...
    private static ErfSubImageInfo BuildSubImageInfo(IfdEntry[] entries, int subIfdLevel, byte[] bytes, bool le)
    {
        ...
        // Epson-compressed RAW = 65535; not yet supported.
        bool canDecode = compression is 1 or 5 or 7 or 8 or 32773 or 32946 && compression != 65535
    ...
    private static ErfMetadata ParseErfMetadata(IfdEntry[] entries, byte[] bytes, bool le)
    {
    ...
    private static ImageMetadata BuildImageMetadata(ErfMetadata epson)
    {
        var tags = new Dictionary<string, string>(StringComparer.Ordinal);
        if (epson.MakerNoteLength > 0)
        {
            tags["ERF:MakerNoteLength"] = epson.MakerNoteLength.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        return new ImageMetadata
        {
            CameraMake = epson.Make,
            CameraModel = epson.Model,
            Software = epson.Software,
            CapturedAtRaw = epson.DateTime,
            Author = epson.Artist,
            Copyright = epson.Copyright,
