...
/// <summary>
/// Reader for Kodak Digital Camera Raw (DCR) files. DCR is TIFF-based,
/// identified at parse time by the EXIF <c>Make</c> tag value beginning
/// with "EASTMAN KODAK COMPANY", "KODAK" or "Kodak" (Kodak's DCS Pro,
/// DCS 14n, EasyShare Pro and digital-back lineups all emit DCR with
/// one of those Make-tag prefixes). The reader composes
/// <see cref="TiffReader"/> for pixel decode and exposes the parsed
/// Kodak-specific metadata block.
/// </summary>
/// <remarks>
/// <para>
/// Like NEF / ARW / PEF, DCR places a small preview / thumbnail in IFD 0
/// and the full-resolution raw sensor data inside a SubIFD pointed at by
/// tag 0x014A. SubIFDs are walked recursively to populate
/// <see cref="SubImages"/>.
/// </para>
/// <para>
/// Kodak-compressed RAW (TIFF compression tag 65000) uses a proprietary
/// per-row predictor + look-up table scheme that Mediar does not yet ship -
...
/// </para>
/// </remarks>
public sealed class DcrReader : IImageReader
{
    ...
    /// <inheritdoc/>
    public ImageFormat Format => ImageFormat.Dcr;
    ...
    /// <summary>The Kodak-specific metadata block parsed from IFD 0.</summary>
    public DcrMetadata Dcr { get; }

    /// <summary>All sub-images discovered in this DCR file (IFD 0 plus SubIFDs in walk order).</summary>
    public IReadOnlyList<DcrSubImageInfo> SubImages { get; }

    private DcrReader(Stream s, bool ownsStream, byte[] bytes, bool le,
                      ImageInfo info, ImageMetadata meta, DcrMetadata dcr,
                      IReadOnlyList<DcrSubImageInfo> subImages, bool canDecode)
    {
        _stream = s; _ownsStream = ownsStream;
        _bytes = bytes; _littleEndian = le;
        Info = info; Metadata = meta; Dcr = dcr;
        SubImages = subImages; CanDecodePixels = canDecode;
    }

    /// <summary>Open a DCR file by path.</summary>
    public static DcrReader Open(string path)
    {
    ...
    /// <summary>Open a DCR from a stream (the contents are buffered into memory).</summary>
    public static DcrReader Open(Stream stream, bool ownsStream = false)
    {
        ArgumentNullException.ThrowIfNull(stream);
        using var ms = new MemoryStream();
        stream.CopyTo(ms);
        var bytes = ms.ToArray();
        if (bytes.Length < 8) throw new ImageFormatException("Truncated DCR.");

        bool le = bytes[0] == 'I' && bytes[1] == 'I';
        bool be = bytes[0] == 'M' && bytes[1] == 'M';
        if (!le && !be) throw new ImageFormatException("Bad DCR byte-order mark (expected II or MM).");
        int magic = ReadU16(bytes, 2, le);
        if (magic != 42) throw new ImageFormatException("Unsupported DCR/TIFF magic " + magic + ".");

        uint ifd0Offset = ReadU32(bytes, 4, le);
        if (ifd0Offset == 0) throw new ImageFormatException("DCR file has no IFDs.");

        var ifd0 = ParseIfd(bytes, le, (int)ifd0Offset);
        var dcr = ParseDcrMetadata(ifd0, bytes, le);

        if (string.IsNullOrEmpty(dcr.Make) || !IsKodakMake(dcr.Make))
        {
            throw new ImageFormatException(
                "Not a DCR file (EXIF Make tag does not identify a Kodak camera).");
        }

        var subs = new List<DcrSubImageInfo>();
        ...
        var meta = BuildImageMetadata(dcr);
        return new DcrReader(stream, ownsStream, bytes, le, info, meta, dcr,
                             subs, primary.CanDecodePixels);
    }

    private static bool IsKodakMake(string make) =>
        make.StartsWith("EASTMAN KODAK", StringComparison.Ordinal) ||
        make.StartsWith("KODAK", StringComparison.Ordinal) ||
        make.StartsWith("Kodak", StringComparison.Ordinal);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        if (!CanDecodePixels)
        {
            throw new NotSupportedException(
                "This DCR file's primary image uses an unsupported compression scheme " +
                "(Kodak-compressed RAW / tag 65000 is not yet implemented).");
        }
        ...
        {
            throw new NotSupportedException(
                "DCR primary image was not produced by the underlying TIFF decoder.");
        }
    }
    ...
    private static DcrSubImageInfo SelectPrimary(IReadOnlyList<DcrSubImageInfo> subs)
    {
        DcrSubImageInfo? best = null;
        long bestPixels = 0;
        ...
        return best ?? throw new ImageFormatException("DCR file has no inspectable sub-images.");
    }

    private static void WalkIfdsRecursive(byte[] bytes, bool le, uint ifdOffset,
                                          int parentSubIfdLevel,
                                          List<DcrSubImageInfo> sink,
    ...
    private static DcrSubImageInfo BuildSubImageInfo(IfdEntry[] entries, int subIfdLevel, byte[] bytes, bool le)
    {
        ...
        // Kodak-compressed RAW = 65000; not yet supported.
    ...
    private static DcrMetadata ParseDcrMetadata(IfdEntry[] entries, byte[] bytes, bool le)
    {
    ...
    private static ImageMetadata BuildImageMetadata(DcrMetadata dcr)
    {
        var tags = new Dictionary<string, string>(StringComparer.Ordinal);
        if (dcr.MakerNoteLength > 0)
        {
            tags["DCR:MakerNoteLength"] = dcr.MakerNoteLength.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        return new ImageMetadata
        {
            CameraMake = dcr.Make,
            CameraModel = dcr.Model,
            Software = dcr.Software,
            CapturedAtRaw = dcr.DateTime,
            Author = dcr.Artist,
            Copyright = dcr.Copyright,
