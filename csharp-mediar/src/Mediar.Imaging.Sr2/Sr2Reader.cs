...
/// <summary>
/// Reader for Sony Cyber-shot v2 RAW (SR2) files. SR2 is TIFF-based, identified at
/// parse time by the EXIF <c>Make</c> tag value beginning with
/// "SONY" or "Sony" (
/// Sony Cyber-shot DSC-R1, DSC-F828, DSC-F717, DSC-V3 (the pre-Alpha advanced compacts before ARW superseded the format)
/// ). The reader composes <see cref="TiffReader"/> for pixel
/// decode and exposes the parsed Sony-specific metadata block.
/// </summary>
/// <remarks>
/// <para>
/// Like NEF / ARW / PEF / DCR, SR2 places a small preview / thumbnail in
...
/// </para>
/// <para>
/// Sony-compressed RAW (TIFF compression tag 32770) uses a 7-of-14 bit packing scheme similar to older ARW files. SR2 files are emitted by the Sony Cyber-shot DSC-R1 / DSC-F828 / DSC-F717 / DSC-V3 family that predate the Alpha DSLR / mirrorless line; the file extension is the sole disambiguator from header bytes alone.
/// 
...
/// </para>
/// </remarks>
public sealed class Sr2Reader : IImageReader
{
    ...
    /// <inheritdoc/>
    public ImageFormat Format => ImageFormat.Sr2;
    ...
    /// <summary>The Sony-specific metadata block parsed from IFD 0.</summary>
    public Sr2Metadata SR2 { get; }

    /// <summary>All sub-images discovered in this SR2 file (IFD 0 plus SubIFDs in walk order).</summary>
    public IReadOnlyList<Sr2SubImageInfo> SubImages { get; }

    private Sr2Reader(Stream s, bool ownsStream, byte[] bytes, bool le,
                     ImageInfo info, ImageMetadata meta, Sr2Metadata sony,
                     IReadOnlyList<Sr2SubImageInfo> subImages, bool canDecode)
    {
        _stream = s; _ownsStream = ownsStream;
        _bytes = bytes; _littleEndian = le;
        Info = info; Metadata = meta; SR2 = sony;
        SubImages = subImages; CanDecodePixels = canDecode;
    }

    /// <summary>Open a SR2 file by path.</summary>
    public static Sr2Reader Open(string path)
    {
    ...
    /// <summary>Open a SR2 from a stream (the contents are buffered into memory).</summary>
    public static Sr2Reader Open(Stream stream, bool ownsStream = false)
    {
        ArgumentNullException.ThrowIfNull(stream);
        using var ms = new MemoryStream();
        stream.CopyTo(ms);
        var bytes = ms.ToArray();
        if (bytes.Length < 8) throw new ImageFormatException("Truncated SR2.");

        bool le = bytes[0] == 'I' && bytes[1] == 'I';
        bool be = bytes[0] == 'M' && bytes[1] == 'M';
        if (!le && !be) throw new ImageFormatException("Bad SR2 byte-order mark (expected II or MM).");
        int magic = ReadU16(bytes, 2, le);
        if (magic != 42) throw new ImageFormatException("Unsupported SR2/TIFF magic " + magic + ".");

        uint ifd0Offset = ReadU32(bytes, 4, le);
        if (ifd0Offset == 0) throw new ImageFormatException("SR2 file has no IFDs.");

        var ifd0 = ParseIfd(bytes, le, (int)ifd0Offset);
        var sony = ParseSr2Metadata(ifd0, bytes, le);

        if (string.IsNullOrEmpty(sony.Make) || !IsSonyMake(sony.Make))
        {
            throw new ImageFormatException(
                "Not a SR2 file (EXIF Make tag does not identify a Sony camera).");
        }

        var subs = new List<Sr2SubImageInfo>();
        ...
        var info = new ImageInfo
        {
            ...
            Format = ImageFormat.Sr2,
            HasAlpha = false,
            FrameCount = 1,
            ColorSpace = "RAW",
        };

        var meta = BuildImageMetadata(sony);
        return new Sr2Reader(stream, ownsStream, bytes, le, info, meta, sony,
                             subs, primary.CanDecodePixels);
    }

    private static bool IsSonyMake(string make) =>
        make.StartsWith("SONY", StringComparison.Ordinal) || make.StartsWith("Sony", StringComparison.Ordinal);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        if (!CanDecodePixels)
        {
            throw new NotSupportedException(
                "This SR2 file's primary image uses an unsupported compression scheme " +
                "(Sony-compressed RAW / tag 32770 is not yet implemented).");
        }
        ...
        {
            throw new NotSupportedException(
                "SR2 primary image was not produced by the underlying TIFF decoder.");
        }
    }
    ...
    private static Sr2SubImageInfo SelectPrimary(IReadOnlyList<Sr2SubImageInfo> subs)
    {
        Sr2SubImageInfo? best = null;
        long bestPixels = 0;
        ...
        return best ?? throw new ImageFormatException("SR2 file has no inspectable sub-images.");
    }

    private static void WalkIfdsRecursive(byte[] bytes, bool le, uint ifdOffset,
                                          int parentSubIfdLevel,
                                          List<Sr2SubImageInfo> sink,
    ...
    private static Sr2SubImageInfo BuildSubImageInfo(IfdEntry[] entries, int subIfdLevel, byte[] bytes, bool le)
    {
        ...
        // Sony-compressed RAW = 32770; not yet supported.
        bool canDecode = compression is 1 or 5 or 7 or 8 or 32773 or 32946 && compression != 32770
    ...
    private static Sr2Metadata ParseSr2Metadata(IfdEntry[] entries, byte[] bytes, bool le)
    {
    ...
    private static ImageMetadata BuildImageMetadata(Sr2Metadata sony)
    {
        var tags = new Dictionary<string, string>(StringComparer.Ordinal);
        if (sony.MakerNoteLength > 0)
        {
            tags["SR2:MakerNoteLength"] = sony.MakerNoteLength.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        return new ImageMetadata
        {
            CameraMake = sony.Make,
            CameraModel = sony.Model,
            Software = sony.Software,
            CapturedAtRaw = sony.DateTime,
            Author = sony.Artist,
            Copyright = sony.Copyright,
