...
/// <summary>
/// Reader for Samsung NX RAW (SRW) files. SRW is TIFF-based, identified at
/// parse time by the EXIF <c>Make</c> tag value beginning with
/// "SAMSUNG TECHWIN", "SAMSUNG ELECTRONICS CO.,LTD.", "SAMSUNG" or
/// "Samsung" (Samsung's NX-series mirrorless cameras and certain Galaxy
/// devices). The reader composes <see cref="TiffReader"/> for pixel
/// decode and exposes the parsed Samsung-specific metadata block.
/// </summary>
/// <remarks>
/// <para>
/// Like NEF / ARW / PEF / DCR, SRW places a small preview / thumbnail in
...
/// </para>
/// <para>
/// Samsung-compressed RAW (TIFF compression tag 32770) uses a proprietary
/// 12/14-bit delta-coded packing scheme that Mediar does not yet ship -
...
/// </para>
/// </remarks>
public sealed class SrwReader : IImageReader
{
    ...
    /// <inheritdoc/>
    public ImageFormat Format => ImageFormat.Srw;
    ...
    /// <summary>The Samsung-specific metadata block parsed from IFD 0.</summary>
    public SrwMetadata Srw { get; }

    /// <summary>All sub-images discovered in this SRW file (IFD 0 plus SubIFDs in walk order).</summary>
    public IReadOnlyList<SrwSubImageInfo> SubImages { get; }

    private SrwReader(Stream s, bool ownsStream, byte[] bytes, bool le,
                     ImageInfo info, ImageMetadata meta, SrwMetadata srw,
                     IReadOnlyList<SrwSubImageInfo> subImages, bool canDecode)
    {
        _stream = s; _ownsStream = ownsStream;
        _bytes = bytes; _littleEndian = le;
        Info = info; Metadata = meta; Srw = srw;
        SubImages = subImages; CanDecodePixels = canDecode;
    }

    /// <summary>Open an SRW file by path.</summary>
    public static SrwReader Open(string path)
    {
    ...
    /// <summary>Open an SRW from a stream (the contents are buffered into memory).</summary>
    public static SrwReader Open(Stream stream, bool ownsStream = false)
    {
        ArgumentNullException.ThrowIfNull(stream);
        using var ms = new MemoryStream();
        stream.CopyTo(ms);
        var bytes = ms.ToArray();
        if (bytes.Length < 8) throw new ImageFormatException("Truncated SRW.");

        bool le = bytes[0] == 'I' && bytes[1] == 'I';
        bool be = bytes[0] == 'M' && bytes[1] == 'M';
        if (!le && !be) throw new ImageFormatException("Bad SRW byte-order mark (expected II or MM).");
        int magic = ReadU16(bytes, 2, le);
        if (magic != 42) throw new ImageFormatException("Unsupported SRW/TIFF magic " + magic + ".");

        uint ifd0Offset = ReadU32(bytes, 4, le);
        if (ifd0Offset == 0) throw new ImageFormatException("SRW file has no IFDs.");

        var ifd0 = ParseIfd(bytes, le, (int)ifd0Offset);
        var srw = ParseSrwMetadata(ifd0, bytes, le);

        if (string.IsNullOrEmpty(srw.Make) || !IsSamsungMake(srw.Make))
        {
            throw new ImageFormatException(
                "Not an SRW file (EXIF Make tag does not identify a Samsung camera).");
        }

        var subs = new List<SrwSubImageInfo>();
        ...
        var meta = BuildImageMetadata(srw);
        return new SrwReader(stream, ownsStream, bytes, le, info, meta, srw,
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        if (!CanDecodePixels)
        {
            throw new NotSupportedException(
                "This SRW file's primary image uses an unsupported compression scheme " +
                "(Samsung-compressed RAW / tag 32770 is not yet implemented).");
        }
        ...
        {
            throw new NotSupportedException(
                "SRW primary image was not produced by the underlying TIFF decoder.");
        }
    }
    ...
    private static SrwSubImageInfo SelectPrimary(IReadOnlyList<SrwSubImageInfo> subs)
    {
        SrwSubImageInfo? best = null;
        long bestPixels = 0;
        ...
        return best ?? throw new ImageFormatException("SRW file has no inspectable sub-images.");
    }

    private static void WalkIfdsRecursive(byte[] bytes, bool le, uint ifdOffset,
                                          int parentSubIfdLevel,
                                          List<SrwSubImageInfo> sink,
    ...
    private static SrwSubImageInfo BuildSubImageInfo(IfdEntry[] entries, int subIfdLevel, byte[] bytes, bool le)
    {
        ...
        // Samsung-compressed RAW = 32770; not yet supported.
    ...
    private static SrwMetadata ParseSrwMetadata(IfdEntry[] entries, byte[] bytes, bool le)
    {
    ...
    private static ImageMetadata BuildImageMetadata(SrwMetadata srw)
    {
        var tags = new Dictionary<string, string>(StringComparer.Ordinal);
        if (srw.MakerNoteLength > 0)
        {
            tags["SRW:MakerNoteLength"] = srw.MakerNoteLength.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        return new ImageMetadata
        {
            CameraMake = srw.Make,
            CameraModel = srw.Model,
            Software = srw.Software,
            CapturedAtRaw = srw.DateTime,
            Author = srw.Artist,
            Copyright = srw.Copyright,
