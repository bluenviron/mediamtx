...
/// <summary>
/// Reader for Sony Alpha RAW (ARW) files. ARW is TIFF-based, identified
/// at parse time by the EXIF <c>Make</c> tag value beginning with "SONY".
/// The reader composes <see cref="TiffReader"/> for pixel decode and
/// exposes the parsed Sony-specific metadata block.
/// </summary>
/// <remarks>
/// <para>
/// Like NEF and DNG, ARW places a small preview / thumbnail in IFD 0 and
...
/// </para>
/// <para>
/// Sony's compressed RAW (TIFF compression tags 32767 / 32769 / 32770) uses
/// proprietary 11-to-7-bit log encoding (ARW v1) or compressed Huffman-style
/// schemes (ARW v2 lossless / lossy) that Mediar does not yet ship -
/// sub-images using those compressions are reported as
...
/// </para>
/// </remarks>
public sealed class ArwReader : IImageReader
{
    ...
    /// <inheritdoc/>
    public ImageFormat Format => ImageFormat.Arw;
    ...
    public ArwMetadata Arw { get; }

    /// <summary>All sub-images discovered in this ARW file (IFD 0 plus SubIFDs in walk order).</summary>
    public IReadOnlyList<ArwSubImageInfo> SubImages { get; }

    private ArwReader(Stream s, bool ownsStream, byte[] bytes, bool le,
                     ImageInfo info, ImageMetadata meta, ArwMetadata arw,
                     IReadOnlyList<ArwSubImageInfo> subImages, bool canDecode)
    {
        _stream = s; _ownsStream = ownsStream;
        _bytes = bytes; _littleEndian = le;
        Info = info; Metadata = meta; Arw = arw;
        SubImages = subImages; CanDecodePixels = canDecode;
    }

    /// <summary>Open an ARW file by path.</summary>
    public static ArwReader Open(string path)
    {
    ...
    /// <summary>Open an ARW from a stream (the contents are buffered into memory).</summary>
    public static ArwReader Open(Stream stream, bool ownsStream = false)
    {
        ArgumentNullException.ThrowIfNull(stream);
        using var ms = new MemoryStream();
        stream.CopyTo(ms);
        var bytes = ms.ToArray();
        if (bytes.Length < 8) throw new ImageFormatException("Truncated ARW.");

        bool le = bytes[0] == 'I' && bytes[1] == 'I';
        bool be = bytes[0] == 'M' && bytes[1] == 'M';
        if (!le && !be) throw new ImageFormatException("Bad ARW byte-order mark (expected II or MM).");
        int magic = ReadU16(bytes, 2, le);
        if (magic != 42) throw new ImageFormatException("Unsupported ARW/TIFF magic " + magic + ".");

        uint ifd0Offset = ReadU32(bytes, 4, le);
        if (ifd0Offset == 0) throw new ImageFormatException("ARW file has no IFDs.");

        var ifd0 = ParseIfd(bytes, le, (int)ifd0Offset);
        var arw = ParseArwMetadata(ifd0, bytes, le);

        if (string.IsNullOrEmpty(arw.Make) ||
            !arw.Make.StartsWith("SONY", StringComparison.Ordinal))
        {
            throw new ImageFormatException(
                "Not an ARW file (EXIF Make tag does not identify a Sony camera).");
        }

        var subs = new List<ArwSubImageInfo>();
        ...
        var meta = BuildImageMetadata(arw);
        return new ArwReader(stream, ownsStream, bytes, le, info, meta, arw,
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        if (!CanDecodePixels)
        {
            throw new NotSupportedException(
                "This ARW file's primary image uses an unsupported compression scheme " +
                "(Sony-compressed RAW / tags 32767 / 32769 / 32770 are not yet implemented).");
        }
        ...
        {
            throw new NotSupportedException(
                "ARW primary image was not produced by the underlying TIFF decoder.");
        }
    }
    ...
    private static ArwSubImageInfo SelectPrimary(IReadOnlyList<ArwSubImageInfo> subs)
    {
        ArwSubImageInfo? best = null;
        long bestPixels = 0;
        ...
        return best ?? throw new ImageFormatException("ARW file has no inspectable sub-images.");
    }

    private static void WalkIfdsRecursive(byte[] bytes, bool le, uint ifdOffset,
                                          int parentSubIfdLevel,
                                          List<ArwSubImageInfo> sink,
    ...
    private static ArwSubImageInfo BuildSubImageInfo(IfdEntry[] entries, int subIfdLevel, byte[] bytes, bool le)
    {
        ...
        // Sony-compressed RAW = 32767 (ARW v1 8-bit packed), 32769 / 32770 (ARW v2);
        // none are supported yet. JPEG-in-TIFF (7) and uncompressed (1) flow through.
    ...
    private static ArwMetadata ParseArwMetadata(IfdEntry[] entries, byte[] bytes, bool le)
    {
    ...
    private static ImageMetadata BuildImageMetadata(ArwMetadata arw)
    {
        var tags = new Dictionary<string, string>(StringComparer.Ordinal);
        if (arw.MakerNoteLength > 0)
        {
            tags["ARW:MakerNoteLength"] = arw.MakerNoteLength.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        return new ImageMetadata
        {
            CameraMake = arw.Make,
            CameraModel = arw.Model,
            Software = arw.Software,
            CapturedAtRaw = arw.DateTime,
            Author = arw.Artist,
