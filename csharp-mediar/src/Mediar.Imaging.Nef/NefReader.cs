...
namespace Mediar.Imaging.Nef;

/// <summary>
/// Reader for Nikon Electronic Format (NEF) RAW files. NEF is TIFF-based,
/// identified at parse time by the EXIF <c>Make</c> tag value
/// "NIKON CORPORATION". The reader composes <see cref="TiffReader"/> for
/// pixel decode and exposes the parsed Nikon-specific metadata block.
/// </summary>
/// <remarks>
/// <para>
/// Like DNG, NEF places a small preview / thumbnail in IFD 0 and the
/// full-resolution raw sensor data inside a SubIFD pointed at by tag
/// 0x014A. SubIFDs are walked recursively to populate <see cref="SubImages"/>.
/// </para>
/// <para>
/// Nikon's compressed RAW (TIFF compression tag 34713) uses a proprietary
/// predictor + Huffman scheme that Mediar does not yet ship - sub-images
...
/// </para>
/// </remarks>
public sealed class NefReader : IImageReader
{
    ...
    /// <inheritdoc/>
    public ImageFormat Format => ImageFormat.Nef;
    ...
    public NefMetadata Nef { get; }

    /// <summary>All sub-images discovered in this NEF file (IFD 0 + SubIFDs in walk order).</summary>
    public IReadOnlyList<NefSubImageInfo> SubImages { get; }

    private NefReader(Stream s, bool ownsStream, byte[] bytes, bool le,
                     ImageInfo info, ImageMetadata meta, NefMetadata nef,
                     IReadOnlyList<NefSubImageInfo> subImages, bool canDecode)
    {
        _stream = s; _ownsStream = ownsStream;
        _bytes = bytes; _littleEndian = le;
        Info = info; Metadata = meta; Nef = nef;
        SubImages = subImages; CanDecodePixels = canDecode;
    }

    /// <summary>Open a NEF file by path.</summary>
    public static NefReader Open(string path)
    {
    ...
    /// <summary>Open a NEF from a stream (the contents are buffered into memory).</summary>
    public static NefReader Open(Stream stream, bool ownsStream = false)
    {
        ArgumentNullException.ThrowIfNull(stream);
        using var ms = new MemoryStream();
        stream.CopyTo(ms);
        var bytes = ms.ToArray();
        if (bytes.Length < 8) throw new ImageFormatException("Truncated NEF.");

        bool le = bytes[0] == 'I' && bytes[1] == 'I';
        bool be = bytes[0] == 'M' && bytes[1] == 'M';
        if (!le && !be) throw new ImageFormatException("Bad NEF byte-order mark (expected II or MM).");
        int magic = ReadU16(bytes, 2, le);
        if (magic != 42) throw new ImageFormatException("Unsupported NEF/TIFF magic " + magic + ".");

        uint ifd0Offset = ReadU32(bytes, 4, le);
        if (ifd0Offset == 0) throw new ImageFormatException("NEF file has no IFDs.");

        var ifd0 = ParseIfd(bytes, le, (int)ifd0Offset);
        var nef = ParseNefMetadata(ifd0, bytes, le);

        if (string.IsNullOrEmpty(nef.Make) ||
            !nef.Make.StartsWith("NIKON", StringComparison.Ordinal))
        {
            throw new ImageFormatException(
                "Not a NEF file (EXIF Make tag does not identify a Nikon camera).");
        }

        var subs = new List<NefSubImageInfo>();
        ...
        var info = new ImageInfo
        {
            ...
            Format = ImageFormat.Nef,
            HasAlpha = false,
            FrameCount = 1,
            ColorSpace = "RAW",
        };

        var meta = BuildImageMetadata(nef);
        return new NefReader(stream, ownsStream, bytes, le, info, meta, nef,
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        if (!CanDecodePixels)
        {
            throw new NotSupportedException(
                "This NEF file's primary image uses an unsupported compression scheme " +
        ...
        {
            throw new NotSupportedException(
                "NEF primary image was not produced by the underlying TIFF decoder.");
        }
    }
    ...
    private static NefSubImageInfo SelectPrimary(IReadOnlyList<NefSubImageInfo> subs)
    {
        NefSubImageInfo? best = null;
        long bestPixels = 0;
        ...
        return best ?? throw new ImageFormatException("NEF file has no inspectable sub-images.");
    }

    private static void WalkIfdsRecursive(byte[] bytes, bool le, uint ifdOffset,
                                          int parentSubIfdLevel,
                                          List<NefSubImageInfo> sink,
    ...
    private static NefSubImageInfo BuildSubImageInfo(IfdEntry[] entries, int subIfdLevel, byte[] bytes, bool le)
    {
    ...
    private static NefMetadata ParseNefMetadata(IfdEntry[] entries, byte[] bytes, bool le)
    {
    ...
    private static ImageMetadata BuildImageMetadata(NefMetadata nef)
    {
        var tags = new Dictionary<string, string>(StringComparer.Ordinal);
        if (nef.MakerNoteLength > 0)
        {
            tags["NEF:MakerNoteLength"] = nef.MakerNoteLength.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        return new ImageMetadata
        {
            CameraMake = nef.Make,
            CameraModel = nef.Model,
            Software = nef.Software,
            CapturedAtRaw = nef.DateTime,
            Author = nef.Artist,
            Copyright = nef.Copyright,
