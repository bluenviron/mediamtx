...
/// <summary>
/// Reader for Nikon NEF camera-RAW files. NEF is a standard
...
/// a thin factory that supplies the NEF-specific
/// <see cref="TiffRawConfig"/>.
/// </para>
/// <para>
/// Nikon-compressed RAW (TIFF compression tag 34713) uses a
...
/// </para>
/// </remarks>
public static class NefReader
{
    /// <summary>NEF <see cref="TiffRawConfig"/>.</summary>
    public static readonly TiffRawConfig Config = new()
    {
        ...
        IsMatchingMake = IsNikonMake,

    };

    /// <summary>Open a NEF file from a stream.</summary>
    public static TiffRawReader Open(Stream stream, bool ownsStream = false)
        => TiffRawReader.OpenStandard(stream, Config, ownsStream);

    /// <summary>Open a NEF file from a filesystem path.</summary>
    ...
    private static bool IsNikonMake(string m)
        => m.StartsWith("NIKON", StringComparison.Ordinal) || m.StartsWith("Nikon", StringComparison.Ordinal);
}