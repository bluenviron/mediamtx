...
func (s *muxerVariantFMP4Segment) finalize(
	...
	if s.videoTrack != nil {
		s.renderedDuration = nextPartVideoSample.DTS - s.startDTS
	} else {
...
func (s *muxerVariantFMP4Segment) writeH264(sample *fmp4.PartVideoSample, adjustedPartDuration time.Duration) error {
	size := uint64(0)
...
func (s *muxerVariantFMP4Segment) writeAAC(sample *fmp4.PartAudioSample, adjustedPartDuration time.Duration) error {
