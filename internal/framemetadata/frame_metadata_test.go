...
func TestInserterH264KeyThenDelta(t *testing.T) {
	...
	out2 := ins.MaybeInsert(
		&format.H264{},
		au2,
		nil,
		now2,
		time.Time{},
		keyPTS+int64(clockRate)*33/1000,
		clockRate,
	).(unit.PayloadH264)
...
func TestPTZFieldsOnlyOnVersionChange(t *testing.T) {
	...
	out2 := ins.MaybeInsert(
		...
		time.Time{},
		keyPTS+int64(clockRate)*40/1000,
		clockRate,
	).(unit.PayloadH264)
	seiPayload2, ok := parseUserDataUnregisteredSEI(true, out2[0])
	require.True(t, ok)
	...
	out3 := ins.MaybeInsert(
		&format.H264{},
		au,
		meta3,
		now.Add(80*time.Millisecond),
		time.Time{},
		keyPTS+int64(clockRate)*80/1000,
		clockRate,
	).(unit.PayloadH264)
